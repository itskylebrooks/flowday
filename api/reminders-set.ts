...
interface Body { initData?: string; daily_enabled?: unknown; daily_time?: unknown; weekly_enabled?: unknown; weekly_day?: unknown; weekly_time?: unknown }

export default async function handler(req: Req, res: Res) {
  try {
    if (req.method !== 'POST') return res.status(405).json({ ok:false, error:'method-not-allowed' });
    if (supabaseInitError) return res.status(500).json({ ok:false, error:supabaseInitError });
    const { initData, daily_enabled, daily_time, weekly_enabled, weekly_day, weekly_time } = (req.body as Body) || {};
    ...
    const weeklyEnabled = !!weekly_enabled;
    const reTime = /^([0-2]\d):([0-5]\d)$/;
    const dt = typeof daily_time === 'string' && reTime.test(daily_time) ? daily_time : '20:00';
    const wt = typeof weekly_time === 'string' && reTime.test(weekly_time) ? weekly_time : '18:00';
    let wd = typeof weekly_day === 'number' ? weekly_day : (typeof weekly_day === 'string' ? parseInt(weekly_day,10) : 1);
    if (isNaN(wd) || wd<0 || wd>6) wd = 1; // store 0..6 app style; convert to 1..7 iso if needed later
    // Upsert
