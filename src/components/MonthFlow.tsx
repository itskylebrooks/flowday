...
export default function MonthFlow({
  ...
  const main = hues
    .slice(0, 3)
    .map((h, i) => ({ color: hsl(h), offset: i / Math.max(1, hues.slice(0, 3).length - 1) }));
  const stops =
    main.length >= 2
      ? main
      : (() => {
          const h = hues[0] ?? 220;
          return [hsl((h + 350) % 360), hsl(h), hsl((h + 10) % 360)].map((c, i) => ({
            color: c,
            offset: i / 2,
          }));
        })();
  ...
  if (empty) {
    return (
      <div
        className={'mx-auto flex items-center justify-center select-none ' + className}
        style={{ width, height }}
      >
        <div
      ...
      <g style={{ mixBlendMode: 'screen' }} mask={`url(#${maskId})`}>
        {Array.from({ length: seedsRef.current!.length }).map((_, i) => (
          <circle
            key={i}
            ref={(el) => {
              if (el) glitterRefs.current[i] = el;
            }}
