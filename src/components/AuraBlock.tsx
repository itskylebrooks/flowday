  ...
  return (
    <div className="mx-auto mt-2 flex flex-col items-center">
      <div className="relative h-56 w-56 rounded-full ring-1 ring-white/10" style={auraBackground(hue)}>
        <motion.div className="absolute inset-0" animate={{ rotate: 360 }} transition={{ duration: 18, repeat: Infinity, ease: 'linear' }}>
          {positions.map((p, i) => (
            <div key={(emojis[i] || '_') + i} className="absolute" style={{ left: p.x, top: p.y, transform: 'translate(-50%, -50%)' }}>
              <div className="text-3xl drop-shadow-[0_0_6px_rgba(255,255,255,0.35)]">{emojis[i] || ''}</div>
            </div>
          ))}
        </motion.div>
      </div>
