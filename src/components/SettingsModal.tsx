...
export default function SettingsModal({
  ...
  async function handlePasteImport() {
    if (!pasteInput || !pasteInput.trim()) {
      alert('Paste some JSON to import');
      return;
    }
    setImporting(true);
    try {
  ...
  const { topEmoji, gradientCSS } = useMemo(() => {
    ...
    if (!monthEntries.length) {
      return {
        topEmoji: '🙂',
        gradientCSS:
          'radial-gradient(circle at 50% 50%, hsl(220 10% 28%) 0%, hsl(220 10% 18%) 75%)',
      };
    }
    const { freq } = emojiStats(monthEntries);
    let best: string | null = null;
    let bestC = -1;
    for (const [emo, c] of freq.entries()) {
      if (c > bestC) {
        best = emo;
        bestC = c;
      }
    ...
    >
      ...
      >
        <div className="mb-8">
          <div className="relative h-12 flex items-center justify-center">
            {onShowGuide && (
              ...
              >
                ...
                <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 translate-y-full text-[10px] text-white/40 font-medium whitespace-nowrap pointer-events-none select-none">
                  App guide
                </div>
              </div>
            )}
            <span className="text-lg font-semibold tracking-wide bg-gradient-to-r from-white to-white/60 bg-clip-text text-transparent">
              Settings
            </span>
            {/* Avatar */}
            <div
              className="absolute right-0 top-1/2 -translate-y-1/2"
              title="Your Flowday avatar (auto-generated)"
            >
          ...
          {isTG && (
            <div className="mt-2 flex justify-center">
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-sky-500/30 text-white/85 ring-1 ring-sky-500/60 font-semibold tracking-wide hover:bg-sky-500/50 transition">
                TG Version
              </span>
            </div>
          )}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white/4 p-4 sm:p-5 rounded-2xl ring-1 ring-white/6 shadow-sm text-sm">
            ...
            <form onSubmit={handleSave} className="mt-1 space-y-3">
              <div>
                <label className="block text-[10px] uppercase tracking-wide text-white/45 mb-2">
                  Username
                </label>
                ...
                <p className="mt-2 text-[11px] text-white/40">
                  Lowercase, 24 chars max. Stored on this device.
                </p>
              </div>
              <div className="mt-1">
                <div className="flex items-center justify-between">
                  <div>
                    <button
                      ...
                      onClick={() => {
                        if (
                          !window.confirm('Delete all Flowday local data? This cannot be undone.')
                        )
                          return;
                        clearAllData();
            ...
            <div className="bg-white/4 p-4 sm:p-5 rounded-2xl ring-1 ring-white/6 shadow-sm text-sm">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <hr className="border-t border-white/6 my-3" />
                  <div className="text-[11px] text-center text-white/40 mt-1">
                    This is a local import/export only feature for the web build. Use JSON files to
                  ...
                  <div className="flex items-center justify-center gap-2 text-[11px] text-white/45">
                    <label
                      ...
                      className={
                        ...
                        (mode === 'merge' ? 'ring-emerald-500/30 bg-emerald-600/8' : 'ring-white/8')
                      }
                    >
                      <input
                        aria-hidden
                    ...
                    <label
                      ...
                      className={
                        'px-2 py-1 rounded-md ring-1 cursor-pointer select-none ' +
                        (mode === 'replace' ? 'ring-red-400/25 bg-red-600/6' : 'ring-white/8')
                      }
                    >
                      <input
                        aria-hidden
                ...
                {preview && (
                  <details className="mt-2 text-left text-[11px] text-white/40">
                    <summary className="cursor-pointer">
                      Preview exported JSON (click to expand)
                    </summary>
                    <pre className="mt-2 max-h-60 overflow-auto text-[11px] text-white/60 p-2 bg-black/20 rounded">
                      {preview}
                    </pre>
                  </details>
                )}
            ...
            <div className="bg-white/4 p-4 sm:p-5 rounded-2xl ring-1 ring-white/6 shadow-sm text-sm">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <hr className="border-t border-white/6 my-3" />
                  <div className="text-[11px] text-center text-white/40 mt-1">
                    Use JSON to move data between devices. Copy or paste JSON in Telegram.
                  </div>
                </div>
              </div>
        ...
        <div className="mt-5">
          <button
            onClick={beginClose}
            className="w-full rounded-2xl px-4 py-3 text-sm font-semibold text-white/90 bg-emerald-500/30 ring-1 ring-emerald-500/60 hover:bg-emerald-500/50 transition"
          >
            Done
          </button>
        </div>
        <div className="mt-6 text-center text-[10px] leading-relaxed text-white/45 relative">
          ...
          <div className="mt-1">
            © {new Date().getFullYear()} Kyle Brooks. All rights reserved.
          </div>
