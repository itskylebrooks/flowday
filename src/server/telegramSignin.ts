...
export async function telegramSigninHandler(req: Req, res: Res) {
  try {
  ...
  if (desired && desired.length < 4) return res.status(400).json({ ok:false, error:'username-too-short' });
    if (desired) {
  ...
  res.json({ ok:true, telegram_id: u.id, username: desired });
  } catch (e) {
