...
import { emojiStats, clamp, last7, todayISO, hsl } from '@lib/utils';
...
export default function ConstellationsPage({
  ...
  const momentumRef = useRef<{ vx: number; vy: number; active: boolean }>({
    vx: 0,
    vy: 0,
    active: false,
  });
  ...
  useEffect(() => {
    ...
    for (const e of entries)
      if (e.date === todayStr) for (const emo of Array.from(new Set(e.emojis))) todaySet.add(emo);
    ...
    triggerRender();
  }, [entries, freq, pair, topEmojis, worldHeight, worldWidth]);
  ...
  }, [entries, worldHeight, worldWidth]);
  ...
  function onPointerMove(e: React.PointerEvent<SVGGElement>) {
    ...
    if (primary) {
      primary.x = clamp(
        nx,
        primary.r + NODE_PADDING - CANVAS_OVERFLOW,
        worldWidth - (primary.r + NODE_PADDING) + CANVAS_OVERFLOW,
      );
      primary.y = clamp(
        ny,
        primary.r + NODE_PADDING - CANVAS_OVERFLOW,
        worldHeight - (primary.r + NODE_PADDING) + CANVAS_OVERFLOW,
      );
      primary.vx = 0;
      primary.vy = 0;
    }
    ...
    for (const g of d.group) {
      ...
      if (vmag > maxVel) {
        node.vx = (node.vx / vmag) * maxVel;
        node.vy = (node.vy / vmag) * maxVel;
      }
    }
    triggerRender();
  }
  ...
  useEffect(() => {
    ...
    const nodes = nodesRef.current;
    if (!nodes.length) {
      // still trigger a repaint for empty state, but no loop
      triggerRender();
      return;
    }
    function step(now: number) {
      ...
      if (!draggingRef.current) {
        ...
        if (FORCES_ENABLED) {
          ...
          for (let i = 0; i < nodes.length; i++) {
            const a = nodes[i];
            const baseX = Math.floor(a.x / cellSize),
              baseY = Math.floor(a.y / cellSize);
  ...
  }, [FORCES_ENABLED, topEmojis, worldHeight, worldWidth]);
  ...
  const recent7Emojis = useMemo(
    () => new Set<string>(last7(entries).flatMap((e) => e.emojis)),
    [entries],
  );
  ...
  function onBgPointerMove(e: React.PointerEvent<SVGSVGElement>) {
    ...
    if (d && d.active && !pinch.active) {
      ...
      if (last && now - last.t > 12) {
        ...
        momentumRef.current.vx =
          ((e.clientX - last.x) * (worldWidth / rect.width)) / (now - last.t);
        momentumRef.current.vy =
          ((e.clientY - last.y) * (worldHeight / rect.height)) / (now - last.t);
  ...
  return (
    <div
      className="mx-auto max-w-sm px-4 h-full select-none"
    ...
    >
      ...
      <div
        key={yearKey}
        className="relative mx-auto mt-3 rounded-xl border border-white/5 bg-black/30 p-3 animate-fadeSwap fd-constellation-backdrop"
      ...
      >
        ...
        {nodes.length === 0 && (
          ...
          >
            <div className="font-poster text-white/18" style={{ fontSize: 170, lineHeight: 1 }}>
              ?
            </div>
          </div>
        )}
    ...
    <div className="mt-3 flex justify-center text-white select-none">
      <div style={{ width }} className="flex gap-3">
        <button
          ...
          className="flex-1 rounded-lg bg-white/5 hover:bg-white/10 active:bg-white/15 ring-1 ring-white/10 transition"
          style={btnStyle}
        >
          <svg
