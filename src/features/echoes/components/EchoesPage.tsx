...
export default function EchoesPage({ entries, yearOffset }: EchoesPageProps) {
  ...
  const withSongs = useMemo(
    () => entries.filter((e) => e.song && (e.song.title || e.song.artist)),
    [entries],
  );
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div
        key={'year-' + yearOffset}
        className="flex-1 overflow-y-auto px-4 pb-10 space-y-10 echoes-year-anim"
      >
        {byMonth.map(([ym, list]) => (
                  ...
                  >
                    <div className="relative z-10 w-full flex items-center justify-center">
                      <div className="cassette-sticker font-tape w-full mx-1 px-2 py-2 leading-snug flex flex-col items-center justify-center gap-1.5">
                        <div
                          className="cassette-row title-row text-[11px] text-black/60 w-full overflow-hidden text-ellipsis whitespace-nowrap"
                          title={title}
                        >
                          {title || '—'}
                        </div>
                        <div
                          className="cassette-row artist-row text-[10px] text-black/50 w-full overflow-hidden text-ellipsis whitespace-nowrap"
                          title={artist}
                        >
                          {artist || ''}
                        </div>
      ...
      {active && (
        <div
          className={
            'fixed inset-0 z-50 flex items-center justify-center settings-overlay ' +
            (closing ? 'closing' : '')
          }
          onClick={beginClose}
        >
          <div
            className={
              'settings-panel bg-[#111] w-full max-w-sm mx-auto rounded-2xl p-6 relative ' +
              (closing ? 'closing' : '')
            }
          ...
          >
            {(() => {
              ...
              return (
                <div className="text-center mb-3 text-sm font-medium tracking-wide text-white/85">
                  {weekday}
                </div>
              );
            })()}
            <div
              className="rounded-xl mb-4 relative overflow-hidden h-[200px] px-6 py-4 flex flex-col cassette-preview"
              style={{
                background: typeof active.hue === 'number' ? hsl(active.hue, 80, 50) : '#3a3a3a',
                ['--reel-color' as unknown as string]:
                  typeof active.hue === 'number' ? reelColor(active.hue) : '#bbb',
              }}
            >
              {/* Animated SVG reels */}
              <div className="pointer-events-none absolute inset-0">
                <div className="reel-spin reel-left" aria-hidden="true">
                  <ReelSVG />
                </div>
                <div className="reel-spin reel-right" aria-hidden="true">
                  <ReelSVG />
                </div>
              </div>
              <div className="relative z-10 text-center px-2 font-tape h-full w-full flex flex-col">
                {(() => {
                  ...
                  return (
                    <div
                      className={cls + ' text-black/60 break-words leading-snug mb-1'}
                      style={style}
                    >
                      {title}
                    </div>
                ...
                {(() => {
                  ...
                  return (
                    <div
                      className="text-[11px] tracking-wide leading-tight mb-1"
                ...
                {active.song?.artist &&
                  (() => {
                    ...
                    return (
                      <div
                        className="text-xs md:text-sm text-black/50 break-words leading-snug mt-1"
                        style={style}
                      >
                        {artist}
                      </div>
            ...
            <div className="flex justify-center">
              <button
                onClick={beginClose}
                className="rounded-md px-4 py-1.5 text-sm font-medium text-white/85 ring-1 ring-white/15 hover:bg-white/5"
              >
                Close
              </button>
            </div>
