...
import ConstellationsPage from '@features/constellations/components/ConstellationsPage';
import EchoesPage from '@features/echoes/components/EchoesPage';
import FlowsPage from '@features/flows/components/FlowsPage';
import PrivacyTelegramPage from '@features/privacy/pages/PrivacyTelegramPage';
import PrivacyWebPage from '@features/privacy/pages/PrivacyWebPage';
import { getRecents, pushRecent } from '@lib/storage';
import { disableVerticalSwipes, enableVerticalSwipes, hapticLight } from '@lib/telegram';
import {
  addDays,
  canEdit,
  clamp,
...
} from './hooks/useTelegramEnvironment';
...
export default function App() {
  ...
  const {
    isTelegram: isTG,
    accentColor: tgAccent,
    footerOffset: footerBottomOffset,
    headerOffset,
  } = useTelegramEnvironment(page);
  const contentTop = HEADER_HEIGHT + headerOffset;
  const contentBottom = FOOTER_HEIGHT + footerBottomOffset;
  const MAX_TITLE = 48;
  const MAX_ARTIST = 40;
  ...
  const [guideOpen, setGuideOpen] = useState(() => {
    try {
      return localStorage.getItem('flowday_seen_guide_v1') ? false : true;
    } catch {
      return true;
    }
  });
  useEffect(() => {
    if (guideOpen === false) {
      try {
        localStorage.setItem('flowday_seen_guide_v1', '1');
      } catch {
        /* ignore */
      }
    }
  ...
  const { entries, entry, replaceEntry } = useEntriesState(activeDate);
  ...
  interface SliderEl extends HTMLDivElement {
    _wheelTO?: ReturnType<typeof setTimeout>;
  }
  ...
  function relativeLabel(unit: 'week' | 'month' | 'year', offset: number): string {
    ...
    if (offset === 1)
      return unit === 'week' ? 'Last week' : unit === 'month' ? 'Last month' : 'Last year';
  ...
  function headerCenterText(): string {
    ...
    if (page === 'flows')
      return flowsMode === 'week'
        ? relativeLabel('week', weekOffset)
        : relativeLabel('month', monthOffset);
  ...
  function handleReset() {
    ...
    if (page === 'flows') {
      if (flowsMode === 'week') setWeekOffset(0);
  ...
  return (
    <div className="app-viewport fixed inset-0 w-full bg-[#0E0E0E] text-white overflow-hidden">
      {/* Header (fixed) */}
      <div
        className="fixed left-0 right-0 z-20 box-border h-14 text-sm text-white/90"
      ...
      <div
        className="absolute inset-x-0 overflow-hidden page-stack"
          ...
          <div className="mx-auto flex h-full max-w-sm flex-col px-4">
            ...
            <div className="h-[320px] w-full flex items-center justify-center">
              <div
                key={activeDate}
                className={
                  'emoji-trans-container w-full flex items-center justify-center animate-emoji-day-swap ' +
                  (showAura ? 'aura-active' : '')
                }
                style={{ maxWidth: 280 }}
              >
                <div
                  className="triangle-view flex items-center justify-center w-full"
                  onClick={() => {
                    if (entry.emojis.length > 0 && editable) {
                      /* maybe future */
                    }
                  }}
                >
            ...
            <div
              ...
              onWheel={() => {
                if (isTG && sliderRef.current) {
                  disableVerticalSwipes();
                  if (sliderRef.current._wheelTO) clearTimeout(sliderRef.current._wheelTO);
                  sliderRef.current._wheelTO = setTimeout(() => enableVerticalSwipes(), 260);
                }
              }}
              className={
                ...
                (editable && entry.emojis.length > 0
                  ? 'ring-1 ring-white/10 hover:shadow-[0_0_0_3px_rgba(255,255,255,0.07)] active:scale-[0.98]'
                  : 'bg-white/10 cursor-not-allowed') +
                (releaseBlocked ? ' pointer-events-none' : '')
              }
              style={{
                boxShadow:
                  editable && entry.emojis.length > 0 && !releaseBlocked
                    ? '0 0 20px 2px rgba(255,255,255,0.07)'
                    : undefined,
              }}
            ...
            >
              ...
              <div
                aria-hidden
                className={
                  'absolute inset-0 rounded-full transition-opacity duration-400 ease-out transform-gpu'
                }
                style={{
            ...
            {!isTG && !showSong && (
              <div className="mt-6 flex justify-center">
                <button
                  type="button"
                  onClick={() => {
                    if (canEditSongMeta) setShowSong(true);
                  }}
            ...
            {isTG && (
              <div className="mt-6 flex justify-center">
                <button
                  type="button"
                  onClick={() => {
                    if (canEditSongMeta) setSongEditorOpen(true);
                  }}
        ...
        >
          <div className="mx-auto w-full max-w-sm flex items-center justify-center gap-10 px-4 text-white/80 h-full">
            ...
            <IconButton
              ...
              active={page === 'constellations'}
              onClick={() => setPage('constellations')}
              accent={isTG ? tgAccent : undefined}
            >
              <svg viewBox="0 0 24 24" className="h-6 w-6" fill="currentColor">
      ...
      {privacyOpen && (
        <div
          className={
            'fixed inset-0 z-50 flex items-stretch sm:items-center justify-center settings-overlay backdrop-blur-sm' +
            (privacyClosing ? ' closing' : '')
          }
          onClick={() => {
            if (privacyClosing) return;
        ...
        >
          <div
            className={
              'w-full h-full sm:h-auto max-w-none sm:max-w-sm rounded-none sm:rounded-2xl bg-[#111] p-6 pt-7 pb-8 ring-1 ring-white/10 overflow-y-auto settings-panel' +
              (privacyClosing ? ' closing' : '')
            }
            style={{
...
function SongEditorOverlay({
  ...
  return (
    <div
      className={
        'fixed inset-0 z-[200] bg-[#0b0b0b] flex flex-col px-6 pt-12 pb-10 song-editor-overlay ' +
        (closing ? 'closing' : '')
      }
    >
      <div className="flex items-center justify-between mb-8">
        <button
          onClick={handleDone}
          className="px-4 py-2 rounded-full bg-white/10 text-sm font-medium text-white/85 hover:bg-white/15 active:bg-white/20 focus:outline-none focus:ring-2 focus:ring-white/30"
        >
          Done
        </button>
